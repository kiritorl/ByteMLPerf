...
class NpuCkptLoader(CoreCkptLoader):
    def __init__(
        self, 
        prefix, model, 
        mp_size=1, mp_rank=0, 
    ...
    def weight_to_device(self, weight : torch.Tensor, non_blocking=False):
        if self.mp_rank == 0:
            weight = weight.npu(non_blocking=non_blocking)
        else:
            cur_device = torch.npu.current_device()
            weight = torch.empty_like(weight, device=f"npu:{cur_device}")
        return weight
    ...
    def broadcast_weight(self, key, device='cpu', non_blocking=False):   
        ...
        tensor_npu = self.weight_to_device(tensor, non_blocking=non_blocking)
        dist.broadcast(tensor_npu, src=0)
        self.state_dict[key] = tensor_npu
